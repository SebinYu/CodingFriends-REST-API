...
public class MailController {
    ...
    public Response notifyApplicationReceived(Authentication authentication, @PathVariable("Studygroup_id") Long Studygroup_id ) {
        return success(mailService.sendmailTo(getUser(authentication), "", Studygroup_id),"/mail/toLeader/"+ Studygroup_id);
    }

    @Operation(summary = "신청자에게 스터디 신청 수락 메일 전송", description = "신청자에게 스터디 신청 수락 메일 전송한다.")
    @PostMapping("/toApplier/accepted/{Studygroup_id}")
    public Response notifyApplicationAccepted(Authentication authentication, @PathVariable("Studygroup_id") Long Studygroup_id ) {
        return success(mailService.sendmailTo(getUser(authentication), "accepted", Studygroup_id),"/mail/toApplier/accepted/"+ Studygroup_id);
    }

    @Operation(summary = "신청자에게 스터디 신청 거절 메일 전송", description = "신청자에게 스터디 신청 거절 메일 전송한다.")
    @PostMapping("/toApplier/rejected/{Studygroup_id}")
    public Response notifyApplicationRejected(Authentication authentication, @PathVariable("Studygroup_id") Long Studygroup_id ) {
        return success(mailService.sendmailTo(getUser(authentication), "rejected", Studygroup_id),"/mail/toApplier/rejected/"+ Studygroup_id);
    }

    @Operation(summary = "다수 인원에게 이벤트 안내 메일 전송", description = "다수 인원에게 이벤트 안내 메일 전송한다.")
    @GetMapping("/toBulkUsers/noticeEvent")
    public CompletableFuture<String> notifyEventToUsers() throws ExecutionException, InterruptedException {
